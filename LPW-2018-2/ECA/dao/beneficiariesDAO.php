...
class beneficiariesDAO
{
    ...
    public function salvar($beneficiaries)
    {
        ...
        try {
            if ($beneficiaries->getIdBeneficiaries() != "") {
                $statement = $pdo->prepare("UPDATE tb_beneficiaries SET str_nis=:str_nis, str_name_person=:str_name_person, cpf=:cpf, rgp=:rgp 
                                                     WHERE id_beneficiaries = :id;");
                $statement->bindValue(":id", $beneficiaries->getIdBeneficiaries());
            } else {
                $statement = $pdo->prepare("INSERT INTO tb_beneficiaries (str_nis, str_name_person,cpf,rgp) 
                                                     VALUES (:str_nis, :str_name_person, :cpf, :rgp)");
            }
            $statement->bindValue(":str_nis", $beneficiaries->getStrNis());
            var_dump($statement);
            $statement->bindValue(":str_name_person", $beneficiaries->getStrNamePerson());
            $statement->bindValue(":cpf", $beneficiaries->getStrCpf());
            $statement->bindValue(":rgp", $beneficiaries->getIntRgp());
    ...
    public function atualizar($beneficiaries)
    {
        global $pdo;
        try {
            $statement = $pdo->prepare("SELECT id_beneficiaries, str_nis, str_name_person, cpf, rgp FROM tb_beneficiaries WHERE id_beneficiaries = :id");
            $statement->bindValue(":id", $beneficiaries->getIdBeneficiaries());
            if ($statement->execute()) {
                ...
                $beneficiaries->setStrCpf($rs->cpf);
                $beneficiaries->setIntRgp($rs->rgp);
    ...
    public function tabelapaginada()
    {
        ...
        $sql = "SELECT id_beneficiaries, str_nis, str_name_person, cpf, rgp FROM tb_beneficiaries LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
     ...
     <tbody>";
        ...
        <td style='text-align: center'>$bene->cpf</td>
        <td style='text-align: center'>$bene->rgp</td>
